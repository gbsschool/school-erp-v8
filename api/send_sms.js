export default async function handler(req, res) {
  ...
  try {
    ...
    if (!api_token || !mobile || !message) return res.status(400).json({ status:false, error:"api_token, mobile and message are required" });
    ...
    return res.status(200).json({ status:true, proxy:"working", whatsbot_status:response.status, response:result });
  } catch (error) {
    return res.status(500).json({ status:false, proxy:"error", error:error.message });
  }
}